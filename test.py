...
class Loadfile:
    ...
    def init_load(self):
        # check all path
        pathes = ['outcsv/delivered', 'outcsv/bounce', 'outcsv/quota', 'outcsv/clean', 'archives']
        for path in pathes:
    ...
    def clean_email(self, fichier):
        try:
            ...
            with open('incsv/' + fichier, 'r') as scv_file:
            ...
            # names of csv files
            filedelivered = 'outcsv/delivered/delivered_' + fichier
            filedbounce = 'outcsv/bounce/bounce_' + fichier
            fileqouta = 'outcsv/quota/quota_' + fichier
            fileclean = 'outcsv/clean/clean_' + fichier
    ...
    def merge_csv(self):
        ...
        for path in pathes:
            ...
            if source_files:
                ...
                with open(path+'/merge.csv', 'w', newline='') as f:
                    f.write(all_together)

                clean_set = []
                with open(path+'/merge.csv', 'r') as scv_file:
                ...
                with open(path+'/finally.csv', 'w', newline='') as finallyfile:
                    order = csv.writer(finallyfile)
                    for email in clean_list:
                        order.writerow([email])
                finallyfile.close()
                os.remove(path + '/merge.csv')
    ...
    def creat_files(self, path, emails, domains):
        ...
        for domain in domains:
            ...
            write_path = os.path.join(path + domain + '/' + domain + '_.csv')
    ...
    def split_file(self):
        # pathes = ['outcsv/delivered', 'outcsv/bounce', 'outcsv/quota', 'outcsv/clean']
        pathes = ['outcsv/delivered', 'outcsv/clean']
        for path in pathes:
            if not os.path.exists(path):
                return
            else:
                fields = sorted(glob(path+"/*.csv"))
                print('fields: ', fields)
                print('fields: ', path+'/')
                for file in fields:
                    self.div_email(os.path.join(path + os.sep + file))
    ...
    def split_domain(self):
        ...
        for path in pathes:
            if not os.path.exists(path):
                ...
                fields = os.listdir(path)
                for file in fields:
                    self.split_email(os.path.join(path + '/' + file))
    ...
    def creat_cvs_by_domain(self, path, list_email, list_domain):
        ...
        for domain in list_domain:
            ...
            with open(os.path.join(path + '/' + domain+'__.csv'), 'w', newline='') as cleanfile:
                order = csv.writer(cleanfile)
